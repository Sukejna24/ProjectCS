...
if __name__ == "__main__":
    ...
    # Function to display the database path
    st.subheader("Database Path Information")
    db_path = "users.db"  # Assuming 'users.db' is stored locally
    if os.path.exists(db_path):
        st.write(f"The database is located at: `{os.path.abspath(db_path)}`")
    else:
        st.write("The database does not exist locally. Please check the configuration.")
