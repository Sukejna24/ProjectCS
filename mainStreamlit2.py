...
def userInfo(mydb):
    ...
    print("user info from spotify:", user["display_name"])
    if not userExists(mydb, user):
        addUser(mydb, user)
    return user["uri"]
...
def get_playlists():
    ...
    playlists_info=[]
    playlists_str = json.dumps(playlists, indent=4)
    st.code(playlists_str, language='json') #st.write('playlists:' + str(playlists) + 'items:', playlists['items'])
    for pl in playlists['items']:
        if pl != None: # may be there are Nones in the items list, then skip None
            plinfo, artists_info = get_playlist(pl['id'], False)
            playlists_info.append((pl, plinfo))
...
def get_all_tracks_from_playlist(playlist_id, clean=True):
    ...
    return tracks, artists
def getTrackAudioFeatures(track_uri):
    print("getTrackAudioFeatures: uri list=", [track_uri])
    track_features = {  "acousticness": 0.00242,  "analysis_url": "https://api.spotify.com/v1/audio-analysis/2takcwOaAZWiXQijPHIx7B",  "danceability": 0.585,
        "duration_ms": 237040,   "energy": 0.842, "id": "2takcwOaAZWiXQijPHIx7B", "instrumentalness": 0.00686, "key": 9, "liveness": 0.0866, "loudness": -5.883,
        "mode": 0, "speechiness": 0.0556, "tempo": 118.211, "time_signature": 4, "track_href": "https://api.spotify.com/v1/tracks/2takcwOaAZWiXQijPHIx7B", "type": "audio_features",
        "uri": "spotify:track:2takcwOaAZWiXQijPHIx7B", "valence": 0.428 } # this is FAKE. Spotify stop providing as of 27. November 2024: sp.audio_features([track_uri]) (deprecated)
    print("get track uri: track_features (depricated by Spotify as of Nov. 27th, 2024) fake values used:")
...
def sp_clean_tracks(tracks,clean=True):
    ...
    for trackInd, track in enumerate(tracks):
        tr = track["track"]
        # print("track:", track, "\r\n\r\n\r\n")
        track_addInfo = getTrackAudioFeatures(tracks[trackInd]["track"]["uri"]) # get additional track features
        track_addInfo["analysis_url"] = "https://api.spotify.com/v1/audio-analysis/" + tracks[trackInd]["track"]["uri"]
        track_addInfo["id"] = tracks[trackInd]["track"]["uri"][len("spotify:track:"):]
        track_addInfo["track_href"] = "https://api.spotify.com/v1/tracks/" + tracks[trackInd]["track"]["uri"]
        track_addInfo["uri"] = tracks[trackInd]["track"]["uri"]
        track["track"].update(track_addInfo)  # merge this with other dictionary entries
        if clean:
...
if st.session_state["signed_in"]:
    ...
    user = sp.current_user()
    if debug: print("passed sp.current_user():", user["display_name"])
    name = user["display_name"]
    username = user["id"]
    debugStr = "";
    if debug:
        str=json.dumps(user, indent=4);
        debugStr = "({username}/{str})".format(username=user['id'], str=str)
    st.markdown("Hallo {user}{debugStr}! Hier sind die Playlists, Top Tracks und Künstler:".format(user=name,debugStr=debugStr))
    ...
    print("connect localhost db")
    mydb = connect()
    print("connected localhost db")
    usrUri = userInfo(mydb)
    print("usrUri in DB:", usrUri)
    persist_playlists(mydb, playlists_info)
    persist_artists(mydb, artists_info1)
    persist_toptracks(mydb, toptracks_info)
    print("Finished!")