...
def main():
    ...
    # Datenbank erstellen und Daten speichern
    ...
    def login_user(username, password):
        ...
        if user and check_password(password, user[2]):
            st.session_state.user_id = user[0]  # Die user_id wird im Session-State gespeichert
            return user[0]  # Gibt die user_id zurück
        return None
    
    # Zugriff auf die Datenbank des Benutzers (Persönliche Songs)
    def load_user_db():
    ...
    if st.session_state.logged_in:
        st.header("Ihre persönliche Playliste")
        ...
        with st.expander("Mix Up", expanded=st.session_state.expander_opened):
            ...
            if "user_songs_df_similar" not in st.session_state:
                st.session_state.user_songs_df_similar = pd.DataFrame()  # Fallback für späteren Zugriff

            # Button nur anzeigen, wenn mindestens 20 Songs im Korb sind
            if len(st.session_state.cart) >= 5:
                if st.button("Ähnliche Songs finden"):
                    ...
                    # In den st.session_state speichern
            ...
            # Speicher-Button wird angezeigt, wenn Songs generiert wurden
            if st.session_state.similar_songs_generated and not st.session_state.save_playlist_clicked:
                if st.button("Playlist speichern"):
                    # Sicherstellen, dass die DataFrame nicht leer ist
                    if not st.session_state.user_songs_df_similar.empty:
                        try:
                            ...
                            st.success("Playlist wurde erfolgreich gespeichert!")
                            st.session_state.similar_songs_generated = False
                        except Exception as e:
                            st.error(f"Fehler beim Speichern der Playlist: {e}")
                    else:
                        st.error("Keine Songs zum Speichern vorhanden!")
